...
class employeeClass:
    def __init__(self, root):
        ...
        self.txt_dob.bind("<KeyRelease>", self.format_dob)
        ...
        self.txt_doj.bind("<KeyRelease>", self.format_doj)
        ...
        # ======================================================Buttons============================================
        self.btn_add=Button(self.root,text="Save",command=self.add,font=("goudy old style",15),bg="#2196f3",fg="white",cursor="hand2")
        self.btn_add.place(x=500,y=305,width=110,height=28)
        self.root.bind("<Return>", lambda event: self.btn_add.invoke())
        self.btn_update=Button(self.root,text="Update",command=self.update,font=("goudy old style",15),bg="#4caf50",fg="white",cursor="hand2")
        self.btn_update.place(x=620,y=305,width=110,height=28)
        self.root.bind("<Alt-space>", lambda event: self.btn_update.invoke())
        self.btn_delete=Button(self.root,text="Delete",command=self.delete,font=("goudy old style",15),bg="#f44336",fg="white",cursor="hand2")
        self.btn_delete.place(x=740,y=305,width=110,height=28)
        self.root.bind("<Delete>", lambda event: self.btn_delete.invoke())
        self.btn_clear=Button(self.root,text="Clear",command=self.clear,font=("goudy old style",15),bg="#607d8b",fg="white",cursor="hand2")
        self.btn_clear.place(x=860,y=305,width=110,height=28)
        self.root.bind("<Escape>", lambda event: self.btn_clear.invoke())
...
# ==================Add function to Search Bar and Search Button=============================================================================================================
    def format_dob(self, event):
        cursor_pos = self.txt_dob.index("insert")
        text = self.txt_dob.get().replace("-", "")  # remove old dashes
        new_text = ""

    # insert dash after 2nd and 4th digit
    ...
    # update entry without moving cursor
        
        self.txt_dob.delete(0, "end")
        self.txt_dob.insert(0, new_text)
        
        #fix cursor position
        if cursor_pos == 2 or cursor_pos == 5:
            cursor_pos += 1  # skip over the dash

    # keep cursor in right place
        if cursor_pos > len(new_text):
            cursor_pos = len(new_text)
            self.txt_dob.icursor(cursor_pos)

    
    
    def format_doj(self, event):
        cursor_pos = self.txt_doj.index("insert")
        text = self.txt_doj.get().replace("-", "")  # remove old dashes
        new_text = ""
    ...
    # update entry without moving cursor
        ...
        self.txt_doj.insert(0, new_text)
    ...
    # keep cursor in right place
        if cursor_pos > len(new_text):
            cursor_pos = len(new_text)
            self.txt_doj.icursor(cursor_pos)
